...
\begin{frame}
Hidden semi-Markov models assumptions:
\begin{itemize}
...
\item<3-> probability of staying in a region depends on its length.
\end{itemize}
\end{frame}
...
\begin{enumerate}
\item<1-> Split 3897 positive proteins with signal peptides in 5 groups.
\item<2-> Chose randomly (without replacement) 3897 proteins without signal peptides and split them in 5 groups.
\item<3-> Perform 5-fold cross-validation.
\item<4-> Repeat steps 1-3 150 times (to assure that each sequence from negative data set was chosen at least once).
\end{enumerate} 
\end{frame}
...
\\[12pt]
Final data set chosen for comparison contain 120 eukaryotic proteins with experimentally confirmed signal peptides and 280 eukaryotic proteins without signal peptides added after 2012.
\end{frame}
