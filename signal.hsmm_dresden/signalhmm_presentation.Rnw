...
\begin{frame}
Hidden semi-Markov models assumptions~\cite{rabiner}:
\begin{itemize}
...
\item<4-> Repeat steps 1-3 150 times (to assure with probability 0.95 that each sequence from negative data set was chosen at least once).
\end{enumerate} 
\end{frame}
