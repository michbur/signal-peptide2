...
#here remove too short signal peptides
pos_seqs <- pos_seqs[sapply(pos_seqs, function(i) attr(i, "sig")[2]) > 15]
...
pre_cleaves <- tolower(t(sapply(pos_seqs, function(seq) {
  cleave_site <- attr(seq, "sig")[2]
  seq[(cleave_site-11):(cleave_site-4)]
})))

post_cleaves <- tolower(t(sapply(pos_seqs, function(seq) {
  cleave_site <- attr(seq, "sig")[2]
  seq[(cleave_site+3):(cleave_site+10)]
})))
...
res_norm <- pblapply(1L:4, function(repetition) {
  ...
  lapply(1L:5, function(fold) {
    ...
    trigrams <- cut(test_features(c(rep(1, length(pos_train)), 
                                rep(0, length(neg_train))), 
                              count_ngrams(all_data[c(pos_train, neg_train), ], 
                                           u = 1L:5, n = 3, pos = TRUE)), 
                    breaks = c(0, 0.05, 1))[[1]]
    
    teragrams <- cut(test_features(c(rep(1, length(pos_train)), 
                                 rep(0, length(neg_train))), 
                               count_ngrams(all_data[c(pos_train, neg_train), ], 
                                            u = 1L:5, n = 4, pos = TRUE)), 
                     breaks = c(0, 0.05, 1))[[1]]
    
    tri_models <- train_models(pos_train, neg_train, all_data, trigrams)
    tetra_models <- train_models(pos_train, neg_train, all_data, teragrams)
    cbind(fold = rep(fold, 6), 
          n = c(rep(3, 3), rep(4, 3)),
          test_models(pos_test, neg_train, all_data, trigrams, tri_models),
          test_models(pos_test, neg_test, all_data, teragrams, tetra_models))
    
  })
})

save(res_norm, file = paste0(pathway, "build_cleave_norm.RData"))
